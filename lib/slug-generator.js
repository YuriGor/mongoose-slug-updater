...
module.exports = function(schema, options) {
  ...
  function schemaTraversal(schema, basePath, cb) {
    ...
    schema.eachPath(function(schemaPath, schemaType) {
      // console.log(basePath+schemaPath+":"+schemaType.constructor.schemaName);
...
async function setSlugs(doc, slugs, options, get, set, findOne) {
  ...
  await Promise.all(
    slugs.map(async slug => {
      ...
      slug.fields.forEach(slugValue => {
        // console.log("get",doc,slugValue);
        values.push(get(doc, slugValue));
      });
      let oldSlug = get(doc, slug.path);
      if (!(slug.unique || slug.unique_slug) && (!slug.uniqueGroup||!slug.uniqueGroup.length)) {
        if (!oldSlug || !slug.permanent) {
          set(doc, slug.path, makeSlug(values, options));
          // console.log('slug set',slug.path,values);
        } else {
