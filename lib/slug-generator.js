...
module.exports = function (schema, options) {
    ...
    schema.eachPath(function (pathname, schemaType) {
        if (schemaType.instance == "String" && schemaType.options && schemaType.options.slug) {
            ...
            slug.padding = schemaType.options.slug_padding_size || 2;
    ...
    schema.pre('save', function (next) {
        ...
        async.each(slugs, function (item, callback) {
            ...
            if (!(item.unique || item.unique_slug)) {
                ...
                makeUniqueSlug(doc, item.name, values, opts, item.padding , function (err, slug) {
                    doc[item.name] = slug;
                    callback();
                })
            }
...
function makeUniqueSlug(doc, field, values, options, padding, next) {
